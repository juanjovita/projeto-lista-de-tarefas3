...
function showTask() {
  let new_li = '';
  array_tarefa.forEach(task, index) => {
    //forEach - monta uma li completa e guarda dentro da variável(new_li)
    ...
    new_li =
      new_li +
      `<li class="item-lista ${task.finish == true ? 'finish' : ''}"> 
  <button id="rocket-button" onclick="finishTask(${index})">
    <i class="fas fa-rocket"></i>
  </button>

  <p class="nome-tarefa ${
    task.finish == true ? 'finish' : ''
  }" id="nome-tarefa">${task.task}</p>
